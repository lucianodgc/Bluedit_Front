...
export class Feed implements OnInit {
  ...
  protected listOrderedByDate = signal<Post[]>([]);
  protected listOrderedByVotes = signal<Post[]>([]);
  ...
  loadPosts(currentUserId: number | null, query: string | null) {
    ...
    this.postService.getPosts(currentUserId, query).subscribe({
      next: (response: ApiResponse) => {
        this.listOrderedByDate.set(response.data);
      },
      error: (err) => {
    ...
    this.postService.getPostsByVotes(currentUserId).subscribe({
      next: (response: ApiResponse) => {
        this.listOrderedByVotes.set(response.data);
      },
      error: (err) => {
        console.error('Error al cargar posts', err);
